 ...
 * @var int Event post id
 */
$time_title = apply_filters( 'tribe_events_single_event_time_title', __( 'Time:', 'alpha' ), $event_id );
...
if ( class_exists( 'Tribe__Events__Main' ) ) {
	$post_id     = Tribe__Events__Main::postIdHelper( $event_id );
	$website_url = tribe_get_event_website_url( $post_id );
}
?>

<div class="tribe-events-meta-group tribe-events-meta-group-details">
	<h4 class="tribe-events-single-meta-title text-primary"> <?php esc_html_e( 'Details', 'alpha' ); ?> </h4>
	<dl>
		...
		if ( tribe_event_is_all_day() && tribe_event_is_multiday() ) :
			...
			<dt class="tribe-events-start-date-label"> <?php esc_html_e( 'Start:', 'alpha' ); ?> </dt>
			<dd>
			...
			<dt class="tribe-events-end-date-label"> <?php esc_html_e( 'End:', 'alpha' ); ?> </dt>
			<dd>
		...
		elseif ( tribe_event_is_all_day() ) :
			...
			<dt class="tribe-events-start-date-label"> <?php esc_html_e( 'Date:', 'alpha' ); ?> </dt>
			<dd>
				<abbr class="tribe-events-abbr tribe-events-start-date published dtstart" title="<?php echo esc_attr( $start_ts ); ?>"> <?php echo esc_html( $start_date ); ?> </abbr>
			</dd>
		...
		elseif ( tribe_event_is_multiday() ) :
			...
			<dt class="tribe-events-start-datetime-label"> <?php esc_html_e( 'Start:', 'alpha' ); ?> </dt>
			<dd>
			...
			<dt class="tribe-events-end-datetime-label"> <?php esc_html_e( 'End:', 'alpha' ); ?> </dt>
			<dd>
			...
			<dd>
				<div class="tribe-events-abbr tribe-events-start-time published dtstart" title="<?php echo esc_attr( $end_ts ); ?>">
					<?php echo alpha_escaped( $time_formatted ); ?>
					<?php if ( $show_time_zone ) : ?>
						<span class="tribe-events-abbr tribe-events-time-zone published "><?php echo esc_html( $time_zone_label ); ?></span>
					<?php endif; ?>
				</div>
		...
		if ( ! empty( $cost ) ) :
			...
			<dt class="tribe-events-event-cost-label"> <?php esc_html_e( 'Cost:', 'alpha' ); ?> </dt>
		...
		tribe_meta_event_tags( sprintf( esc_html__( '%s Tags:', 'alpha' ), tribe_get_event_label_singular() ), ', ', true );
		?>
