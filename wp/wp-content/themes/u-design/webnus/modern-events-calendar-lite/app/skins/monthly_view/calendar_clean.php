...
$events_str = '';
if ( $this->display_all ) {
	$events_str .= '<h3 class="mec-table-side-title">' . esc_html__( 'Events', 'alpha' ) . '</h3>';
}
?>
<dl class="mec-calendar-row">
	...
	for ( $x = 0; $x < $running_day; $x++ ) {
		...
		if ( isset( $events[ $today ] ) and count( $events[ $today ] ) ) {
			...
			foreach ( $events[ $today ] as $event ) {
				...
				$events_str .= '<div class="mec-event-content">';

				$events_str .= $after_time_filter;
				$events_str .= '<h4 class="mec-event-title">' . $this->display_link( $event ) . $this->main->get_flags( $event ) . $this->main->get_normal_labels( $event, $display_label ) . $this->main->display_cancellation_reason( $event, $reason_for_cancellation );
				if ( has_filter( 'mec_monthly_virtual_badge' ) ) {
					$events_str .= apply_filters( 'mec_monthly_virtual_badge', $events_str, $event->data->ID );
				}
				$events_str .= '</h4>';
				...
				if ( $this->localtime ) {
					$events_str .= $this->main->module( 'local-time.type3', array( 'event' => $event ) );
				}
	...
	for ( $list_day = 1; $list_day <= $days_in_month; $list_day++ ) {
		...
		if ( isset( $events[ $today ] ) and count( $events[ $today ] ) ) {
			...
			if ( ! $this->display_all ) {
				$events_str .= '<div class="mec-calendar-events-sec" id="mec-calendar-events-sec-' . $this->id . '-' . $day_id . '" data-mec-cell="' . $day_id . '" ' . ( ( trim( $selected_day ) != '' or $this->display_all ) ? ' style="display: block;"' : '' ) . '>' . $this->day_label( $time );
				$events_str .= '<article class="mec-event-article mec-event-empty">';
				$events_str .= '<div class="mec-event-detail"><i class="u-calendar-empty"></i>' . __( 'No Events', 'alpha' ) . '</div>';
				$events_str .= '</article>';
				$events_str .= '</div>';
			}
		}
...
<div class="mec-calendar-events-side mec-clear">
	<?php echo alpha_escaped( $events_str ); ?>
</div>
	<?php
else :
