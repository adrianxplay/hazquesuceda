...
if ( ! class_exists( 'Alpha_Layout_Builder_Admin' ) ) {
	class Alpha_Layout_Builder_Admin extends Alpha_Base {
		...
		public function enqueue_scripts() {
			...
			// Parsing as array.
			foreach ( $conditions as $category => $conditions ) {
				if ( ! class_exists( 'WooCommerce' ) && in_array( $category, array( 'archive_product', 'single_product', 'cart', 'checkout' ) ) ) {
					continue;
				}
				if ( ! alpha_get_option( 'enable_portfolio' ) && ( 'single_' . ALPHA_NAME . '_portfolio' == $category || 'archive_' . ALPHA_NAME . '_portfolio' == $category ) ) {
					continue;
				}
				if ( ! alpha_get_option( 'enable_member' ) && ( 'single_' . ALPHA_NAME . '_member' == $category || 'archive_' . ALPHA_NAME . '_member' == $category ) ) {
					continue;
				}
			...
			wp_localize_script(
				...
				apply_filters(
					...
					array(
						...
						'preview_post_type'             => isset( $_REQUEST['post'] ) && get_post_meta( $_REQUEST['post'], 'preview', true ) ? get_post_meta( $_REQUEST['post'], 'preview', true ) : 'post',
						...
						'text_my_templates'             => esc_html__( 'Select a Template', 'alpha' ),
						'text_duplicate'                => esc_html__( 'Duplicate', 'alpha' ),
						'text_options'                  => esc_html__( 'Edit Conditions', 'alpha' ),
		...
		public function view_layout_builder() {
			...
			$desc  = esc_html__( 'Create a remarkable site with fully customizable layouts and your UDesign templates.', 'alpha' );

			if ( isset( $_REQUEST['layout'] ) && ! empty( $_REQUEST['is_elementor_preivew'] ) ) {
				$template_type = $_REQUEST['layout'];
				$post_type     = get_post_meta( (int) $_REQUEST['post'], 'preview', true );
				$post_type     = $post_type ? $post_type : 'post';

				if ( $template_type ) {
					$title = esc_html__( 'Display Conditions', 'alpha' );
					$desc  = esc_html__( 'Create or choose page layouts you wish to show your editing template, click on the corresponding area and select your editing template.', 'alpha' );
				}
			}
			...
			<script type="text/template" id="alpha_layout_template">
				<div class="alpha-layout">
					...
					<div class="layout-part general" data-part="general">
						<span><?php esc_html_e( 'Page Width, Popup ...', 'alpha' ); ?></span>
					</div>
			...
			<div class="alpha-admin-panel-row" id="alpha_layout_builder">
				...
				<?php endif; ?>
					<div id="alpha_layout_content" class="alpha-layouts-container<?php echo ( isset( $template_type ) && 'block' != $template_type ) ? ' display-conditions' : ''; ?>"></div>
				</div>
			</div>
			<?php
