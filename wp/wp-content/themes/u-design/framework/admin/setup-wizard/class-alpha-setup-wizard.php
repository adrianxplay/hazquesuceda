...
if ( ! class_exists( 'Alpha_Setup_Wizard' ) ) :
	...
	class Alpha_Setup_Wizard extends Alpha_Base {
		...
		public $woopages = array(
			...
			'woocommerce_compare_page_id'   => 'Compare',
		...
		public function view_setup_wizard() {
			if ( ! Alpha_Admin::get_instance()->is_registered() ) {
				if ( isset( $_REQUEST['step'] ) && 'demo_content' == $_REQUEST['step'] ) {
					set_transient( '_alpha_register_redirect', admin_url( 'admin.php?page=alpha-setup-wizard&step=demo_content' ) );
				} elseif ( isset( $_REQUEST['step'] ) && 'default_plugins' == $_REQUEST['step'] ) {
					set_transient( '_alpha_register_redirect', admin_url( 'admin.php?page=alpha-setup-wizard&step=default_plugins' ) );
				} else {
		...
		public function reset_menus() {
			...
			if ( current_user_can( 'manage_options' ) ) {
				...
				$nav_menus = wp_get_nav_menus( array( 'meta_key' => '_alpha_demo' ) );
				$menus     = array();
		...
		public function import_options() {
			...
			if ( current_user_can( 'manage_options' ) ) {
				...
				try {
					...
					alpha_import_theme_options( ( isset( $_REQUEST['import_dummy'] ) && 'true' == $_REQUEST['import_dummy'] ), $options['theme'] );
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'alpha_import_theme_options' ) ) {
	function alpha_import_theme_options( $import_with_dummy, $imported_options ) {
		if ( ! $import_with_dummy ) {
			$options = get_theme_mods();
			if ( isset( $options['conditions'] ) ) {
				$imported_options['conditions'] = $options['conditions'];
			} else {
				unset( $imported_options['conditions'] );
			}
		}
