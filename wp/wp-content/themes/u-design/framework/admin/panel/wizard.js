...
(function (wp, $) {
				...
				if (typeof response.url != 'undefined') {
					...
					if (response.hash == current_item_hash) {
						...
						if ($current_node.find('.demo-plugin').length) {
							...
							$current_node.siblings().find('.demo-plugin').css('pointer-events', '');
						}
						find_next();
					} else {
		...
		return {
			...
			demoInit: function ($el) {
				if ($el) {
					...
					$el.closest('li').find('.d-loading').css('visibility', 'visible');
					$el.closest('li').siblings().find('.demo-plugin').css('pointer-events', 'none');
				}
	...
	var Wizard = {
		...
		doLoading: function ($el) {
			...
			$el.append('<span class="d-loading"><i></i></span>');
		},

		endLoading: function ($el) {
			$el.children('.d-loading').remove();
		...
		initUI: function () {
			$(document.body)
				...
				.on('click', '.button-recommendations', function (e) {
					e.preventDefault();

					var btn = e.currentTarget,
						$btn = $(btn),
						data = $btn.data('recommend');

					Wizard.doLoading($btn);

					setTimeout(function () {
						$('.alpha-wizard-content input[type="checkbox"]').prop('checked', false);

						if (data) {
							var options = alpha_optimize_wizard_params.recommendations[data],
								optionNames = Object.keys(options);

							optionNames.forEach(function (optionName) {
								if (typeof options[optionName] == 'object') {
									var subOptions = options[optionName];
									for (var subOption in subOptions) {
										$('[name="' + optionName + '"][value="' + subOptions[subOption] + '"]').prop('checked', true);
										$('[name="' + optionName + '[]"][value="' + subOptions[subOption] + '"]').prop('checked', true);
									}
								} else if (options[optionName] === true) {
									$('[name="' + optionName + '"]').prop('checked', true);
								}
							})
						}

						Wizard.endLoading($btn);
					}, 350);
				})
	...
	var SetupWizard = {
		...
		removeAlertLeavePage: function () {
			$('.alpha-import-yes.btn-primary').hasClass('disabled') || $('.alpha-import-yes.btn-primary').prop('disabled', false);
		...
		alpha_import_options: function (options) {
			...
			if (options.import_options) {
				var data = { 'action': 'alpha_import_options', 'demo': options.demo_slug, 'import_dummy': $('#alpha-import-dummy').is(':checked'), 'wpnonce': alpha_setup_wizard_params.wpnonce };
