...
if ( class_exists( 'WP_Importer' ) ) {
	class Alpha_WP_Import extends WP_Importer {
				...
				foreach ( $this->posts as $post ) {
					if ( 'ptu_tax' == $post['post_type'] ) { // Post Types Unlimited Compaibility
						...
						if ( $acf_instances ) {
							...
							$tax_args = $acf_instances['ACF_Taxonomy']->get_taxonomy_args( $taxonomy );
							if ( ! taxonomy_exists( $taxonomy['taxonomy'] ) && function_exists( 'alpha_register_taxonomy' ) ) {
								alpha_register_taxonomy( $taxonomy['taxonomy'], (array) $taxonomy['object_type'], $args );
							}
						}
					}
				}
		...
		function process_posts( $index = 0 ) {
			...
			foreach ( $this->posts as $post ) {
				if ( 'ptu' == $post['post_type'] ) { // Post Types Unlimited Compaibility
					foreach ( $post['postmeta'] as $meta ) {
						if ( '_ptu_name' == $meta['key'] ) {
							$this->whitelist[] = $meta['value'];
						}
					}
				} elseif ( 'acf-post-type' == $post['post_type'] ) { // ACF Compatibility
					$this->whitelist[] = maybe_unserialize( $post['post_content'] )['post_type'];
				}
			}
			...
			if ( ! post_type_exists( $post['post_type'] ) && ! in_array( $post['post_type'], $this->whitelist ) ) {
				printf(
		...
		function process_menu_item( $item ) {
			...
			} elseif ( 'post_type_archive' == $_menu_item_type && ( post_type_exists( $_menu_item_object ) || in_array( $_menu_item_object, $this->whitelist ) ) ) {
				$_menu_item_object_id = intval( $_menu_item_object_id );
		...
		function process_menus_orphans( $index = 0 ) {
			...
			$parent_id                = $menu_item_orphans_values[ $index ] ?? 0;
			$menu_item_orphans_keys   = array_keys( $this->menu_item_orphans );
			$child_id                 = $menu_item_orphans_keys[ $index ] ?? 0;
