...
<ul
	...
	if ( $wishlist && $wishlist->has_items() ) :
					...
					<div class="item-wrapper">
						...
						<div class="item-details">
							<div class="item-details-wrapper">
								<a class="close" href="#" title="<?php esc_attr_e( 'Close', 'alpha' ); ?>">&times;</a>
										...
										<?php if ( $show_price || $show_price_variations ) : ?>
											<tr class="product-price">
												...
												<td class="value">
													<?php
													if ( $show_price ) {
														echo alpha_strip_script_tags( $item->get_formatted_product_price() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
													}

													if ( $show_price_variations ) {
														echo alpha_strip_script_tags( $item->get_price_variation() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
													}
													?>
												</td>
											</tr>
								...
								<div class="product-add-to-cart">
									...
									<!-- Quickview Button -->
									<?php echo '<button class="btn btn-quickview btn-outline btn-default btn-rounded btn-dark btn-md" data-mfp-src="' . alpha_get_product_featured_image_src( $product ) . '" data-product="' . $product->get_id() . '" title="' . esc_attr__( 'Quick View', 'alpha' ) . '">' . esc_html__( 'Quick View', 'alpha' ) . '</button>'; ?>
									...
									<?php if ( $show_add_to_cart && isset( $stock_status ) && 'out-of-stock' != $stock_status ) : ?>
										...
										woocommerce_template_loop_add_to_cart(
											array(
												...
												'class'    => 'btn btn-dark btn-rounded btn-dark btn-md',
											)
										);
										?>
									<?php endif ?>
									...
									<div class="move-to-another-wishlist">
										...
										<?php else : ?>
											<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
												<?php echo esc_html( apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'alpha' ) ) ); ?>
											</a>
										<?php endif; ?>
									</div>
								<?php endif; ?>
								...
								<?php if ( $show_remove_product ) : ?>
									<div class="product-remove">
										<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'alpha' ) ) ); ?>"><i class="fa fa-trash"></i></a>
									</div>
								<?php endif; ?>
		...
		<li class="wishlist-empty">
			<?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'alpha' ) ) ); ?>
		</li>
	<?php endif; ?>
</ul>
...
<?php if ( ! empty( $page_links ) ) : ?>
	<nav class="wishlist-pagination">
		<?php echo alpha_strip_script_tags( $page_links ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
	</nav>
<?php endif ?>
