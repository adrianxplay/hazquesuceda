...
<div class="yith-wcwl-add-button">
	<span class="feedback">
		<?php echo alpha_strip_script_tags( $icon ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php echo esc_html( apply_filters( 'yith_wcwl_in_your_wishlist_label', __( 'In your wishlist', 'alpha' ) ) ); ?>
	</span>

	<?php if ( $show_view ) : ?>
		<a href="<?php echo esc_url( $found_in_list->get_url() ); ?>" class="view-wishlist"><?php echo esc_html( apply_filters( 'yith_wcwl_view_wishlist_label', __( 'View &rsaquo;', 'alpha' ) ) ); ?></a>
		<span class="separator"><?php esc_html_e( 'or', 'alpha' ); ?></span>
	<?php endif; ?>

	<!-- WISHLIST POPUP OPENER -->
	<a href="#add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" rel="nofollow" class="move_to_another_wishlist <?php echo esc_attr( $link_classes ); ?> open-pretty-photo" data-rel="prettyPhoto[add_to_wishlist_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>]" data-title="<?php echo esc_attr( apply_filters( 'yith_wcwl_move_to_another_wishlist_title', __( 'Move to another wishlist', 'alpha' ) ) ); ?>" >
		<?php echo ( ! $is_single && 'before_image' == $loop_position ) ? $icon : false; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php echo '<span>' . wp_kses_post( $label ) . '</span>'; ?>
	</a>
	...
	<div id="add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" class="yith-wcwl-popup">
		...
		">
			<div class="yith-wcwl-popup-content">
				...
				<p class="form-row">
					<?php echo wp_kses_post( sprintf( __( 'This item is already in the <b>%s wishlist</b>.<br/>You can move it to another list:', 'alpha' ), $found_in_list->get_formatted_name() ) ); ?>
				</p>
						...
						<p class="form-row form-row-wide">
							<select name="new_wishlist_id" class="wishlist-select">
								<?php if ( ! empty( $lists ) ) : ?>
									...
									<?php endforeach; ?>
								<?php endif; ?>
								<option value="new"><?php echo esc_html( apply_filters( 'yith_wcwl_create_new_list_text', __( 'Create a new list', 'alpha' ) ) ); ?></option>
							</select>
						</p>
					</div>
				</div>
				...
				<div class="yith-wcwl-second-row">
					<p class="form-row form-row-wide">
						<input name="wishlist_name" type="text" class="wishlist-name input-text" placeholder="<?php echo apply_filters( 'yith_wcwl_new_list_title_text', __( 'Wishlist name', 'alpha' ) ); ?>"/>
					</p>
					<p class="form-row form-row-wide">
						<label>
			...
			<div class="yith-wcwl-popup-footer">
				<a rel="nofollow" class="wishlist-submit move_to_wishlist <?php echo esc_attr( $link_popup_classes ); ?>" data-product-id="<?php echo esc_attr( $product_id ); ?>" data-original-product-id="<?php echo esc_attr( $parent_product_id ); ?>" data-item-id="<?php echo ! $found_item ? '' : esc_attr( $found_item->get_id() ); ?>" data-origin-wishlist-id="<?php echo ! $found_in_list ? '' : esc_attr( $found_in_list->get_token() ); ?>">
					<?php echo apply_filters( 'yith_wcwl_move_label', __( 'Move', 'alpha' ) ); ?>
				</a>

				<div class="yith-wcwl-remove-button">
					<i class="fa fa-trash"></i>
					<?php esc_html_e( 'or', 'alpha' ); ?>
					<a href="#" class="delete_item" data-item-id="<?php echo esc_attr( $found_item->get_id() ); ?>" data-product-id="<?php echo esc_attr( $product_id ); ?>" ><?php esc_html_e( 'click to remove it', 'alpha' ); ?></a>
				</div>
			</div>
