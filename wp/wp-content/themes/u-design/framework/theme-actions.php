...
// Compatibility with WooCommerce 8.5.x
add_filter( 'woocommerce_create_pages', 'alpha_woo_default_page_contents' );
...
add_action( 'wp_ajax_alpha_load_popup', 'alpha_load_popup' );
add_action( 'wp_ajax_nopriv_alpha_load_popup', 'alpha_load_popup' );
		...
		if ( isset( $_POST['args'] ) && isset( $_POST['props'] ) ) {
			...
			if ( 'alpha-posts-grid' == $shortcode ) {
				...
				foreach ( $args as $key => $value ) {
					...
					$args_str .= ' ' . sanitize_key( $key ) . '="' . esc_attr( json_encode( $value ) ) . '"';
				}
...
if ( ! function_exists( 'alpha_load_popup' ) ) {
	 ...
	 * @since 1.0
	 */
	function alpha_load_popup() {
		//check_ajax_referer( 'alpha-nonce', 'nonce' );
		// phpcs:disable WordPress.Security.NonceVerification.NoNonceVerification

		if ( isset( $_POST['popups'] ) && is_array( $_POST['popups'] ) ) {
			$popups = $_POST['popups'];
			if ( ! empty( $popups ) ) {
				$result = array();
				foreach ( $popups as $popup ) {
					ob_start();
					alpha_print_popup_template( $popup, '', '' );
					$result[ $popup ] = ob_get_clean();
				}
...
/**
 * Replace wp block contents with shortcode
 *
 * @since 1.3.0
 */
if ( ! function_exists( 'alpha_woo_default_page_contents' ) ) {
	function alpha_woo_default_page_contents( $pages ) {
		if ( isset( $pages['cart'] ) ) {
...
if ( ! function_exists( 'alpha_ajax_print_popup' ) ) {
	...
	function alpha_ajax_print_popup() {
		...
		if ( $id ) {
			alpha_print_popup_template( esc_attr( $id ), '', '' );
		}
