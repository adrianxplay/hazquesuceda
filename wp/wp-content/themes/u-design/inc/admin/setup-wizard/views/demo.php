					...
					if ( Alpha_Admin::get_instance()->is_registered() ) :
						...
						<div class="alpha-install-section alpha-wrap">
							...
							<div>
								...
								<button class="btn btn-primary alpha-import-yes<?php echo alpha_escaped( ! $server_status ? ' disabled' : '' ); ?>"<?php echo alpha_escaped( ! $server_status ? ' disabled="disabled"' : '' ); ?>><?php esc_html_e( 'Standard Import', 'alpha' ); ?></button>
		...
		<?php foreach ( $demos as $demo => $demo_details ) : ?>
			...
			if ( ! empty( $demo_details['plugins'] ) ) {
				foreach ( $demo_details['plugins'] as $plugin ) {
					if ( isset( $all_plugins[ $plugin ]['url'] ) && is_plugin_inactive( $all_plugins[ $plugin ]['url'] ) ) {
