...
<li <?php $section->main_class(); ?>
	...
	<div class="section-header">
		<div class="section-left">
			<h5 class="section-title">
				...
				echo ! $title ? _x( 'Untitled', 'template title empty', 'alpha' ) : $title;
				?>
				
				<?php
				if ( $user->has_enrolled_course( $section->get_course_id() ) ) :
					echo '<sup>' . sprintf( '%d/%d', $user_course->get_completed_items( '', false, $section->get_id() ), $section->count_items( '', true ) ) . '</sup>';
				endif;
				?>
				...
				<?php if ( $description ) : ?>
					<p class="section-desc"><?php echo alpha_strip_script_tags( $description ); ?></p>
				<?php endif; ?>
			</h5>
	...
	<?php if ( ! $items ) : ?>
		<?php learn_press_display_message( __( 'No items in this section', 'alpha' ) ); ?>
	<?php else : ?>
		...
		<ul class="section-content">
			...
			foreach ( $items as $item ) :
				...
				<li class="<?php echo esc_attr( implode( ' ', $item->get_class() ) ); ?>" data-id="<?php echo esc_attr( $item->get_id() ); ?>">
					...
					<a class="section-item-link" href="<?php echo ! $item_link ? 'javascript:void(0);' : esc_url( $item_link ); ?>">
