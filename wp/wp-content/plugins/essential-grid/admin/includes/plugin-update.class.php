...
class Essential_Grid_Plugin_Update
{
	...
	public function do_update_process()
	{
		...
		if (version_compare($this->version, '3.1.2.2', '<')) {
			$this->update_version('3.1.2.2');
			$this->set_version('3.1.2.2');
		}
...
.#handle# .esg-pagination-button:last-child { 
	...
	protected function _update_grids_3014()
	{
		...
		foreach ($grids as $grid) {
			...
			$wpdb->update($table_name, array('params' => json_encode($grid->params)), array('id' => $grid->id));
		}
	}
...
.flat-light .esg-navigationbutton.esg-loadmore {
	...
	protected function _update_3016_watermarks()
	{
		...
		$options = get_option($original_option, array());
		...
		$grids->clear_essential_grids_cache();
	...
	public function update_to_3017()
	{
		...
		if (function_exists('is_multisite') && is_multisite()) {
			...
			foreach ($blogids as $blog_id) {
				...
				$this->_update_3017_grid_by_source_type('esg-socialmedia-addon', 'stream', array('behance', 'facebook', 'flickr', 'instagram', 'twitter'));
				$this->_update_3017_grid_by_source_type('esg-videoplaylists-addon', 'stream', array('vimeo', 'youtube'));
				restore_current_blog();
			}
		} else {
			$upgrade->_retrieve_version_info(true);
	 ...
	 * @return void
	 */
	protected function _update_3017_grid_by_source_type($handle, $source_type, $stream_source_type = array())
	{
		$grids = new Essential_Grid();
		$arrGrids = $grids->get_essential_grids(false, false);
		foreach ($arrGrids as $grid) {
