...
class Essential_Grid_Item_Element
{
	...
	public static function update_essential_item_element($data)
	{
		...
		$response = $wpdb->update($table_name, array('settings' => json_encode($data['settings'])), array('handle' => $data['handle']));
	...
	public static function insert_essential_item_element($data)
	{
		...
		$table_name = Essential_Grid_Db::get_table('elements');
		$response = $wpdb->insert($table_name, array('name' => $data['name'], 'handle' => $data['handle'], 'settings' => json_encode($data['settings'])));
