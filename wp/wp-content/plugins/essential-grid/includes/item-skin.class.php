...
class Essential_Grid_Item_Skin
{
	...
	private $add_css_tags = array(); //example usage: $this->add_css_tags[$unique_class]['a'] = true; //this will give the inner a tags styling information
	...
	public static function update_save_item_skin($data)
	{
		...
		if (empty($data['params'])) return esc_attr__('No parameters found.', ESG_TEXTDOMAIN);
		if (empty($data['layers'])) $data['layers'] = array(); //allow empty layers
	...
	public function get_video_poster_src($media_type)
	{
		...
		foreach ($this->default_video_poster_order as $order) {
			...
			switch ($order) {
				...
				case 'youtube-image':
					if (!in_array($media_type, array('youtube', 'content-youtube'))) {
						$found = false;
						break;
					}
					...
					if (isset($this->layer_values['custom-image-url'][0])) {
						...
						if (empty($this->media_sources[$media_type])) {
							$found = false;
							break;
						}
						// fallback to std URL
						$video_poster_src = '//i.ytimg.com/vi/' . esc_attr($this->media_sources[$media_type]) . '/maxresdefault.jpg';
					}
					break;
				...
				case 'vimeo-image':
					if (!in_array($media_type, array('vimeo', 'content-vimeo')) || empty($this->media_sources[$media_type])) {
						$found = false;
						break;
					}
					$video = json_decode(wp_remote_fopen('https://vimeo.com/api/v2/video/' . esc_attr($this->media_sources[$media_type]) . '.json'));
					if (is_null($video)) {
						$found = false;
						break;
					}
	...
	public function get_media_html($media_source_order)
	{
		...
		if (!empty($media_source_order) && is_array($media_source_order)) {
	...
	public function output_item_skin($demo = false, $choosen_skin = 0)
	{
		...
		if ($this->load_more_element) $li_class .= ' eg-newli';
		...
		if ($demo == 'overview' || $demo == 'skinchoose') {
			...
			if ($demo == 'overview') {
				...
				echo '<a href="admin.php?page=' . ESG_PLUGIN_SLUG . '&view=grid-item-skin-editor&create=' . $this->id . '" class="eg-edit-skin-button eg-overview-button revyellow eg-tooltip-wrap" target="_blank" title="' . esc_attr__('Edit Skin', ESG_TEXTDOMAIN) . '"><i class="eg-icon-tint"></i></a>';
		...
		if ($link_set_to !== 'none') {
			...
			switch ($link_type_link) {
				...
				case 'url':
					...
					$link_wrapper = '<a href="' . esc_attr($this->normalize_link($lurl)) . '"' . $link_target . $link_rel_nofollow . '>%REPLACE%</a>';
					break;
				case 'meta':
					if ($demo === false) {
		...
		if ($demo == 'overview' || $demo == 'skinchoose') {
			echo '            <div class="esg-entry-media' . $media_animation . '"' . $media_animation_delay . $media_animation_duration . $media_blur . $data_transition_media . '><img class="esg-entry-media-img" src="' . ESG_PLUGIN_URL . 'admin/assets/images/' . $this->cover_image . '" data-no-lazy="1"></div>' . "\n\n";
		} else {
			...
			if ($cover_type == 'full' && $c_layer > 0 || ($t_layer > 0 || $c_layer > 0 || $b_layer > 0)) {
				...
				if ($link_set_to == 'cover' && $link_type_link !== 'none') {
					if (strpos($link_wrapper, 'class="') !== false) {
						echo str_replace(array('%REPLACE%', 'class="'), array(esc_html($this->get_item_title()), 'class="eg-invisiblebutton '), $link_wrapper);
					} else {
						echo str_replace(array('%REPLACE%', '<a '), array(esc_html($this->get_item_title()), '<a class="eg-invisiblebutton" '), $link_wrapper);
					}
					$link_inserted = true;
				}
			}
	...
	public function output_add_more()
	{
		echo apply_filters('essgrid_output_add_more', '<li class="filterall eg-addnewitem-wrapper ui-state-disabled">
			<div class="esg-media-cover-wrapper">
				<div class="esg-entry-media"><img class="esg-entry-media-img" src="' . ESG_PLUGIN_URL . 'public/assets/images/300x200transparent.png" data-no-lazy="1"></div>
	...
	public function register_layer_css($layer = false, $demo = false)
	{
		...
		if ($layer === false) {
			 ...
			 * NOTE: most of the if(isset($layer['settings']['source'])){} could be removed if it is known, that there can not be ANY <a> tag in it. Then it can just be replaced with a placeholdertext
			 */
			$m = new Essential_Grid_Meta();
			$text = '';
			...
			switch ($layer['settings']['source']) {
				...
				case 'text':
					...
					if (class_exists('DOMDocument') && defined('LIBXML_HTML_NOIMPLIED') && defined('LIBXML_HTML_NODEFDTD')) {
						$dom = new DOMDocument();
						$dom->loadHTML('<root>' . htmlspecialchars_decode(htmlentities($text)) . '</root>', LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
	...
	public function get_post_value($handle, $separator, $function, $meta, $catmax = '-1', $taxonomy = "")
	{
		...
		//change global post, so the the_content filter use the current post
		$the_post = get_post($base->getVar($this->post, 'ID'));
		if (isset($the_post) && !empty(($the_post))){
			$post = $the_post;
		}
		setup_postdata( $post );
		...
		switch ($handle) {
			...
			case 'excerpt':
				...
				if (empty($text)) {
					...
					$text = apply_filters('the_content', $base->strip_essential_shortcode($base->getVar($this->post, 'post_content')));
				...
				if (empty($postCatsIDs) && isset($this->post['post_type'])) {
					...
					if (!empty($obj) && is_array($obj)) {
						foreach ($obj as $tax) {
							...
							if($use_taxonomies === false) $use_taxonomies = array();
							$use_taxonomies[] = $tax;
	...
	{
		$link = trim($link);
		if (empty($link) || $link == 'javascript:void(0);' || strpos($link, 'mailto:') !== false) return $link;
