const AlphaAjaxSelect2Control = function ( props ) {
	...
	useEffect(
		() => {
			...
			const path = alpha_block_vars.site_url + '/wp-json/ajaxselect2/v1/' + option + '/';
			$el.select2( {
				ajax: {
					url: path,
					dataType: 'json',
					data: function ( params ) {
						...
						if ( condition_name ) {
							args['condition'] = $( 'select[data-setting="' + condition_name + '"]' ).val();
						}
						return args;
					}
				},
	...
	return (
		<div className="components-base-control alpha-ajaxselect2">
...
export default AlphaAjaxSelect2Control;