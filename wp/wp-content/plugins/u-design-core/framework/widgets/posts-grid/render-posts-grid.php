...
$default_atts = array(
	...
	'col_cnt_widescreen'   => '',
	'col_cnt_laptop'       => '',
	'col_cnt_tablet_extra' => '',
	'col_cnt_tablet'       => '',
	'col_cnt_mobile_extra' => '',
	'col_cnt_mobile'       => '',
	// 'col_cnt_min'          => '',
	'list_col_cnt'         => '',
	'col_sp'               => '',
...
if ( 'archive' == $shortcode_type ) {
	...
	if ( 'product' == $post_type ) {
		...
		if ( isset( $_COOKIE[ ALPHA_NAME . '_gridcookie' ] ) && 'list' == $_COOKIE[ ALPHA_NAME . '_gridcookie' ] && ! empty( $list_builder_id ) ) {
			$builder_id_backup = $builder_id;
			$builder_id        = $list_builder_id;
			$list_builder_id   = $builder_id_backup;
			$toggle_cols       = array(
				'col_cnt_widescreen'   => $col_cnt_widescreen,
				'col_cnt_laptop'       => $col_cnt_laptop,
				'col_cnt'              => empty( $col_cnt ) ? 4 : (int) $col_cnt,
				'col_cnt_tablet_extra' => $col_cnt_tablet_extra,
				'col_cnt_tablet'       => $col_cnt_tablet,
				'col_cnt_mobile_extra' => $col_cnt_mobile_extra,
				'col_cnt_mobile'       => $col_cnt_mobile,
			);

			$view                   = 'grid';
			$atts['col_cnt']        = (int) $list_col_cnt;
			$atts['col_cnt_mobile'] = 1;
			unset( $atts['col_cnt_tablet_extra'], $atts['col_cnt_tablet'], $atts['col_cnt_mobile'], $atts['col_cnt_mobile_extra'], $atts['col_cnt_widescreen'], $atts['col_cnt_laptop'] );
		} else {
			$toggle_cols = array(
...
if ( empty( $shortcode_type ) ) {
	if ( 'terms' == $source ) { // terms
		...
		if ( (int) $is_related ) {
			...
			if ( $post_tax ) {
				$terms             = wp_get_post_terms( get_the_ID(), $post_tax, array( 'fields' => 'id=>slug' ) );
				$args['tax_query'] = array(
					array(
						'taxonomy' => sanitize_text_field( $post_tax ),
						'field'    => 'slug',
	...
	if ( $builder_post ) {
		...
		if ( $css ) {
			if ( alpha_is_elementor_preview() ) {
				...
				if ( alpha_doing_ajax() ) {
					echo '<style id="block_' . (int) $builder_id . '_css">';
					echo wp_strip_all_tags( $css );
					echo '</style>';
				} else {
		...
		if ( $page_js ) {
			if ( alpha_is_elementor_preview() ) {
				...
				if ( alpha_doing_ajax() ) {
					echo '<script id="block_' . (int) $builder_id . '_js">';
