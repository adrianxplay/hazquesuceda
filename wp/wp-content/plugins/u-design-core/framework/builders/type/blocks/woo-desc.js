...
( function ( wpI18n, wpBlocks, wpBlockEditor, wpComponents ) {
    ...
    const AlphaTBWooDesc = function ( { attributes, setAttributes, name, clientId } ) {
        ...
        if ( attributes.el_class ) {
            ...
            selectorCls = 'alpha-tb-woo-desc-' + Math.ceil( Math.random() * 10000 );
        ...
        if ( attributes.alignment || attributes.font_settings ) {
            ...
            internalStyle += alphaGenerateTypographyCSS( fontAtts, selectorCls + ' p' );
        }
    ...
    registerBlockType( alpha_admin_vars.theme + '-tb/' + alpha_admin_vars.theme + '-woo-desc', {
        title: __( 'Woo Short Description', 'alpha-core' ),
        icon: 'alpha',
