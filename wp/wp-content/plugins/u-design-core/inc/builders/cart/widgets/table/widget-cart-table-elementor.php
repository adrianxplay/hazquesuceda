		...
		<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
			...
			<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
				...
				<tbody>
					...
					<tr>
						<td colspan="6" class="actions pe-0">
							<div class="cart-actions">
								<a href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>" class="btn btn-dark btn-icon-left continue-shopping btn-border-thin mb-2 <?php echo is_rtl() ? 'ms-auto' : 'me-auto'; ?>"><?php echo is_rtl() ? '' : '<i class="' . ALPHA_ICON_PREFIX . '-icon-long-arrow-left"></i>'; ?><?php esc_html_e( 'Continue shopping', 'woocommerce' ); ?><?php echo is_rtl() ? '<i class="' . ALPHA_ICON_PREFIX . '-icon-long-arrow-right"></i>' : ''; ?></a>
								<?php if ( 'yes' == $settings['cart_show_clear'] ) : ?>
									<button type="submit" class="btn btn-outline btn-default btn-border-thin clear-cart-button mb-2" name="clear_cart" value="<?php esc_attr_e( 'Clear cart', 'alpha-core' ); ?>"><?php esc_html_e( 'Clear cart', 'alpha-core' ); ?></button>
								<?php endif; ?>
								<?php if ( 'yes' != $settings['cart_auto_update'] ) : ?>
									<button type="submit" class="btn btn-outline btn-default btn-border-thin wc-action-btn ms-2 mb-2" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'woocommerce' ); ?>"><?php esc_html_e( 'Update cart', 'woocommerce' ); ?></button>
								<?php endif; ?>
							</div>
							...
							<?php if ( wc_coupons_enabled() ) { ?>
								<div id="cart_coupon_box" style="display: none;">
									<div class="form-row form-coupon">
										<input type="text" name="coupon_code" class="input-text form-control mb-4" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Enter coupon code here...', 'alpha-core' ); ?>">
										<button type="submit" name="apply_coupon" class="btn btn-border-thin btn-outline btn-dark" value="<?php esc_attr_e( 'Apply coupon', 'woocommerce' ); ?>"><?php esc_html_e( 'Apply coupon', 'woocommerce' ); ?></button>
